...
def genQ1FigAcc(epochs, train_acc, test_acc):
    ...
    plt.ylim(0, 1)
    plt.legend()
    plt.savefig("./proj_figures/PartA_1a_1_" + str(epochs) + "_iters.png")
    plt.close()


def genQ1FigLoss(epochs, train_loss):
    plt.plot(range(epochs+1), train_loss, label="training loss")
    plt.text(epochs, train_loss[-1], "%.4f" % train_loss[-1])
    plt.xlabel(str(epochs) + ' iterations')
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig("./proj_figures/PartA_1a_2_" + str(epochs) + "_iters.png")
    plt.close()
...
with tf.Session() as session:
    ...
    for step in range(no_epochs):
        ...
        if step in snapshot:
            genQ1FigAcc(step, train_acc, test_acc)
            genQ1FigLoss(step, train_loss)