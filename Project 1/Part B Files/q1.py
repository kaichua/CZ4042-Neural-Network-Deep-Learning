...
loss = tf.reduce_mean(tf.reduce_sum(tf.square(y - y_), axis = 1) + beta*regularization)
...
with sess.as_default():
	...
	for i in range(1, epochs+1):
		...
		for j in range(1, trainX.shape[0] // 8):
			...
			cumTrainError += loss.eval(feed_dict={x: trainX[lastJ:j*8], y_: trainY[lastJ:j*8]})
			lastJ = j*8
		...
		for j in range(1, testX.shape[0] // 8):
			cumTestError += loss.eval(feed_dict={x: testX[lastJ:j*8], y_: testY[lastJ:j*8]})
			lastJ = j*8
...
with sess.as_default():
	for i in range(1, 8):
		pred = y.eval(feed_dict={x: testX[lastI:i*8]})
		for j in range(8):
