...
def main():
  ...
  inputs, logits = rnn_char_model(x)
  ...
  with tf.Session() as sess:
    ...
    for epoch in range(NUM_EPOCHS):
      ...
      if epoch%1000 == 0:
        genAccLossFig3(epoch, train_loss, train_acc, test_acc)
        pd.DataFrame(train_loss).to_csv('/content/gdrive/My Drive/CZ4042 Dataset/csv/Q3/Q3train_loss_'+str(epoch)+'.csv')
        pd.DataFrame(train_acc).to_csv('/content/gdrive/My Drive/CZ4042 Dataset/csv/Q3/Q3train_acc_'+str(epoch)+'.csv')
        pd.DataFrame(test_acc).to_csv('/content/gdrive/My Drive/CZ4042 Dataset/csv/Q3/Q3test_acc_'+str(epoch)+'.csv')
        end = time.time()
        print("Time used at %d: %f"%(epoch, end))
