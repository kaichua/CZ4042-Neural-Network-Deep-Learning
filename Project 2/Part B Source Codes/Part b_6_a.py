...
def main():
  ...
  for model in learning:
    ...
    inputs, logits = rnn_words_model(x, num_words, model)
    #inputs, logits = rnn_char_model(x, model)
    ...
    with tf.Session() as sess:
      ...
      for epoch in range(NUM_EPOCHS):
        ...
        if epoch%100 == 0:
          genAccLossFig6_2(epoch, train_loss, train_acc, test_acc, '_'+str(model))
          pd.DataFrame(train_loss).to_csv('/content/gdrive/My Drive/CZ4042 Dataset/csv/Q6/A/Q4train_loss_'+str(epoch)+'_'+str(model)+'.csv')
          pd.DataFrame(train_acc).to_csv('/content/gdrive/My Drive/CZ4042 Dataset/csv/Q6/A/Q4train_acc_'+str(epoch)+'_'+str(model)+'.csv')
          pd.DataFrame(test_acc).to_csv('/content/gdrive/My Drive/CZ4042 Dataset/csv/Q6/A/Q4test_acc_'+str(epoch)+'_'+str(model)+'.csv')
    print("Completed %s" % model)
  print("==========END===========")
