...
def main():
  ...
  inputs, logits = rnn_words_model(x, num_words)
  ...
  with tf.Session() as sess:
    ...
    for epoch in range(NUM_EPOCHS):
      ...
      if epoch%100 == 0:
        genAccLossFig4(epoch, train_loss, train_acc, test_acc)
        pd.DataFrame(train_loss).to_csv('/content/gdrive/My Drive/CZ4042 Dataset/csv/Q4/Q4train_loss_'+str(epoch)+'.csv')
        pd.DataFrame(train_acc).to_csv('/content/gdrive/My Drive/CZ4042 Dataset/csv/Q4/Q4train_acc_'+str(epoch)+'.csv')
        pd.DataFrame(test_acc).to_csv('/content/gdrive/My Drive/CZ4042 Dataset/csv/Q4/Q4test_acc_'+str(epoch)+'.csv')
        end = time.time()
