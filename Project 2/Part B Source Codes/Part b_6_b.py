...
def rnn_char_model_layers(x, dropout = 0.0):
  ...
  if dropout > 0:
   stacked_rnn_cell = tf.contrib.rnn.DropoutWrapper(stacked_rnn_cell, input_keep_prob=dropout, output_keep_prob=dropout)

  _, encoding = tf.nn.static_rnn(stacked_rnn_cell, char_list, dtype=tf.float32)
...
def rnn_words_model_layers(x, num_words, dropout = 0.0):
  ...
  _, encoding = tf.nn.static_rnn(stacked_rnn_cell, word_list, dtype=tf.float32)
...
def main():
  ...
  inputs, logits = rnn_words_model_layers(x, num_words)
  #inputs, logits = rnn_char_model_layers(x)
  ...
  with tf.Session() as sess:
    ...
    for epoch in range(NUM_EPOCHS):
      ...
      if epoch%100 == 0:
        genAccLossFig6_2(epoch, train_loss, train_acc, test_acc, '_layered')
        pd.DataFrame(train_loss).to_csv('/content/gdrive/My Drive/CZ4042 Dataset/csv/Q6/B/Q4train_loss_'+str(epoch)+'_layered.csv')
        pd.DataFrame(train_acc).to_csv('/content/gdrive/My Drive/CZ4042 Dataset/csv/Q6/B/Q4train_acc_'+str(epoch)+'_layered.csv')
        pd.DataFrame(test_acc).to_csv('/content/gdrive/My Drive/CZ4042 Dataset/csv/Q6/B/Q4test_acc_'+str(epoch)+'_layered.csv')
